...
for q in range(0,2):
    input_txt_o1 = './2000_200/cup_obj_path'+str(q)+'.txt'
    input_txt_g = './2000_200/cup_path'+str(q)+'.txt'
    input_txt_o = './drawing/real/real/input_txt_o.txt'
    ...
    for i in range (len(rz_o)):
        ...
        # print('=================')
        # print(co)
        circle = plt.Circle((x_o[i],y_o[i]), r/2,color=(co,0,0), fill=True, alpha=0.5)
        # angle=(rz_o[i]-r)*180/m.pi
        # print(angle)
        # rect = mp.Rectangle(((x_o[i]-((m.sqrt(2)*a*m.sin(m.pi/4+rz_o[i]))/2)), (y_o[i]-m.sqrt(2)*a*m.cos(m.pi/4+rz_o[i])/2)), a, a, color=(co,0,0), alpha=0.5, angle=-(rz_o[i]*180/m.pi))
    ...
    #plt.scatter(x_o,y_o,s = 700,c = abs(y_pred-y_o),cmap = 'Reds_r',alpha = 0.7)
    plt.scatter(x_o, y_o, c='b', marker='.')  # 点
    ...
    plt.savefig('./2000_200/'+str(q)+'_real_cup.png')
    plt.close()
