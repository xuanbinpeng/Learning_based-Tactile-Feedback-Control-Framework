...
class ObjectPushEnv(BaseObjectEnv):
    def __init__(
        ...
        self.fix_obj = env_modes["fix_obj"]
        ...
        # obj info
        self.globalscale = 0.93
        self.obj_width = 0.08 * self.globalscale
        self.obj_height = 0.08 * self.globalscale
        ...
        if self.arm_type in ['mg400', 'magician']:
            ...
            TCP_lims[2, 0], TCP_lims[2, 1] = -0.0, 0.0  # z lims
            TCP_lims[3, 0], TCP_lims[3, 1] = -0.0, 0.0  # roll lims
            TCP_lims[4, 0], TCP_lims[4, 1] = - 45 * np.pi /180 , 45 * np.pi / 180  # yaw lims TODO
            TCP_lims[5, 0], TCP_lims[5, 1] = - 45 * np.pi /180 , 45 * np.pi / 180  # yaw lims
    ...
    def setup_action_space(self):
        ...
        if self.control_mode == "TCP_position_control":
            ...
            self.z_act_min, self.z_act_max = -max_pos_change, max_pos_change  #TODO
            self.roll_act_min, self.roll_act_max = -max_ang_change, max_ang_change  #TODO
            self.pitch_act_min, self.pitch_act_max = -max_ang_change, max_ang_change #TODO
    ...
    def setup_object(self):
        ...
        #self.object_path = add_assets_path("rl_env_assets/nonprehensile_manipulation/object_push/green_cup/model.urdf")
    ...
    def reset_object(self):
        ...
        if self.rand_obj_mass:
            self.obj_mass = self.np_random.uniform(0.26, 0.5)
            self.lateralFriction = self.np_random.uniform(0.01, 0.1)
            self.spinningFriction = 0.0
            self._pb.changeDynamics(self.obj_id, -1, 
                                    mass=self.obj_mass,
    ...
    def update_trajectory_simplex(self):
        ...
        """
        # initialise noise
        simplex_noise = OpenSimplex(seed=self.np_random.integers(1e7))
    ...
    def update_trajectory_sin(self):
            ...
            y = np.sin((x - init_offset) / 0.2 * PI) * 0.025
            if flip:
                y = -y
            z = np.zeros_like(x)
            for i in range(int(self.traj_n_points)):
                self.traj_pos_workframe[i] = [x[i],y[i],z[i]]
    ...
    def update_goal(self):
        ...
        if self.targ_traj_list_id >= self.traj_n_points:
            ...
            self.final_goal_id = self.traj_ids[self.traj_n_points-1]
            # get final goal pose in world frame
            (
                self.final_goal_pos_worldframe,
                self.final_goal_orn_worldframe,
            ) = self._pb.getBasePositionAndOrientation(self.final_goal_id)      
            self.final_goal_rpy_worldframe = self._pb.getEulerFromQuaternion(self.final_goal_orn_worldframe)
            ...
            if self.targ_traj_list_id + 1 < len(self.traj_rpy_workframe):
                self.goal_next_pos_workframe = self.traj_pos_workframe[self.targ_traj_list_id+1]
                self.goal_next_orn_workframe = self.traj_orn_workframe[self.targ_traj_list_id+1]
                self.goal_next_rpy_workframe = self.traj_rpy_workframe[self.targ_traj_list_id+1]
            else:
                self.goal_next_pos_workframe = self.traj_pos_workframe[self.targ_traj_list_id]
                self.goal_next_orn_workframe = self.traj_orn_workframe[self.targ_traj_list_id]
                self.goal_next_rpy_workframe = self.traj_rpy_workframe[self.targ_traj_list_id]                
    ...
    def encode_TCP_frame_actions(self, actions):
        ...
        elif self.movement_mode == "TxRyRz":
    ...
    def encode_actions(self, actions):
        ...
        if self.movement_mode in ["y", "yRz", "xyRz","xRyRz"]:
            encoded_actions = self.encode_work_frame_actions(actions)
        elif self.movement_mode in ["TyRz", "TxTyRz","TxRyRz"]:
    ...
    def dense_reward(self):
        ...
        unachieved_goals = (self.traj_n_points - self.targ_traj_list_id) / self.traj_n_points 
        #obj_final_goal_dist = self.xyz_obj_dist_to_final_goal()
        #print(unachieved_goals)
        # weights for rewards
        W_obj_goal_pos = 1.0
        W_obj_goal_orn = 1.0
        W_tip_obj_orn = 0.8
        #W_obj_final_goal_pos = 1.0
        W_unachieved_goals = 1
        #print(unachieved_goals*0.07+obj_goal_pos_dist )
        # sum rewards with multiplicative factors
        reward = -(
            (W_obj_goal_pos * obj_goal_pos_dist) + (W_obj_goal_orn * obj_goal_orn_dist) + (W_tip_obj_orn * tip_obj_orn_dist) # + (W_unachieved_goals * unachieved_goals ) 
        )
    ...
    def get_oracle_obs(self):
        ...
        self.cur_obj_pos_workframe, self.cur_obj_orn_workframe = self.get_obj_pos_workframe()
        self.cur_obj_rpy_workframe = self._pb.getEulerFromQuaternion(self.cur_obj_orn_workframe)
        (
        ...
        observation = np.hstack(
            [
                ...
                *self.goal_next_pos_workframe,
                *self.goal_next_rpy_workframe,
            ]
        )
    ...
    def get_privilege_obs(self):
        ...
        """
        # get sim info on object
        self.cur_obj_pos_workframe, cur_obj_orn_workframe = self.get_obj_pos_workframe()
        self.cur_obj_rpy_workframe = self._pb.getEulerFromQuaternion(cur_obj_orn_workframe)
        (
            cur_obj_lin_vel_workframe,
            cur_obj_ang_vel_workframe,
        ) = self.get_obj_vel_workframe()
