...
class BaseObjectEnv(BaseTactileEnv):
    ...
    def load_object(self, visualise_goal=True):
        ...
        #TODO 
        self.obj_id = self._pb.loadURDF(self.object_path, self.init_obj_pos, self.init_obj_orn,globalScaling = self.globalscale, useFixedBase=False)
        #TODO self._pb.changeDynamics(self.obj_id, -1, mass=0.001,lateralFriction=10.0)   
    ...
    def xyz_obj_dist_to_final_goal(self):
        """
        xyz L2 distance from the current obj position to the goal.
        """
        dist = np.linalg.norm(self.cur_obj_pos_worldframe - self.final_goal_pos_worldframe)
        return dist
