...
parser.add_argument('--data-tag', default='tactile1', type=str, help='files with data_tag in name under data directory will be considered as collected states')
...
mu: The mean vector of the learned latent distribution for the input data.
logsigma: The logarithm of the variance of the learned latent distribution for the 
input data.
recon_x: The reconstructed data batch from the VAE.
beta: A hyperparameter that balances the influence of the KL divergence term in the overall 
loss. 
The KL divergence term encourages the latent distribution to be close to a standard Gaussian distribution. 
A higher beta value places more emphasis on this regularization term
'''
...
def main():
    ...
    #model_G = torch.load("checkpoints/model.pt")
